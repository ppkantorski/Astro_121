#Gratefully copied from: http://code.google.com/p/astrolibpy/source/browse/astrolib/baryvel.py?r=5ce313e4db7453c4d5f585beb9b3a335cd47892f
...
def baryvel(dje, deq):
   """
    ...
    EXAMPLE:
          ...
          In [1]: jd = jdcnv(1994, 2, 15, 0)          #==> JD = 2449398.5
          In [2]: vh, vb = baryvel(jd, 2000)          #Original algorithm
                  ==> vh = [-17.07243, -22.81121, -9.889315]  #Heliocentric km/s
                  ==> vb = [-17.08083, -22.80471, -9.886582]  #Barycentric km/s
          In [3]: vh,vb = baryvel(jd, 2000, jpl=True)   #JPL ephemeris
                  ==> vh = [-17.07236, -22.81126, -9.889419]  #Heliocentric km/s
                  ==> vb = [-17.08083, -22.80484, -9.886409]  #Barycentric km/s
   
          In [4]: ra = (19 + 50/60. + 46.77/3600.)*15*np.pi/180.    #RA  in radians
          In [5]: dec = (08 + 52/60. + 3.5/3600.)*np.pi/180.        #Dec in radians
          In [6]: v = (vb[0]*np.cos(dec)*np.cos(ra) +    #Project velocity toward star
    ...
    REVISION HISTORY:
          ...
          Translated to Python, April 2014
   """
